...
check(30, 'Each store is recorded at a usable depth, not a token presence', () => {
  ...
  if (thin.length) {
    fail(`recorded depth per store — ${report.join('; ')}.\n          Short: ${thin.join('; ')}.\n          Each store needs a median of 12+ candidates AND at least 80% of the list recorded. Depth alone is satisfiable by recording a few items well and skipping the rest; coverage alone is satisfiable by recording two candidates for everything. Neither tells you whether the matcher works at that store.\n          The corpus is now untracked, so the 256KB publishing limit no longer constrains it — record the real shop properly, and let reality-sample.json stay the trimmed tracked file. An item a store genuinely does not stock is a passing outcome; record it as absent rather than padding.`);
  }
});
