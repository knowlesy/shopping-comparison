...
async function runExpandedTestSuite() {
  ...
  const results = {
    ...
    dataset50Lists: { totalLists: dataset.length, totalItems: 0, totalMatches: 0, totalAlts: 0, passed: 0, failed: 0, anomalies: [] },
  ...
  const REQUIRED_VARIETIES = [
    { name: "Eggs", subCategory: "eggs", minCount: 4, desc: "Pack sizes (6, 10/12, 15), Large, Organic, Free Range" },
    { name: "Greek Yogurt", subCategory: "yogurt", keyword: "greek", minCount: 4, desc: "0% Fat Free, Authentic, Greek Style, Fage, 500g, 1kg" },
    { name: "Milk", subCategory: "milk", minCount: 5, desc: "1 Pint, 2 Pints, 4 Pints, 6 Pints, Whole, Semi, Skimmed, Cravendale" },
    { name: "Potatoes", subCategory: "potatoes", minCount: 5, desc: "Baby New, Maris Piper, King Edward, Charlotte, Baking, White, Red" },
    { name: "Mince", subCategory: "beef", minCount: 3, desc: "5% Lean, Standard, Frozen, 500g, 1kg" }
  ];
  ...
  for (const store of SUPERMARKETS) {
    ...
    for (const req of REQUIRED_VARIETIES) {
      ...
      if (matches.length >= req.minCount) {
        ...
        results.catalogVariety.details.push(`✓ [${store.toUpperCase()}] ${req.name}: ${matches.length} varieties (min required ${req.minCount})`);
      } else {
        results.catalogVariety.failed++;
        results.catalogVariety.details.push(`✗ [${store.toUpperCase()}] ${req.name}: only ${matches.length} varieties (expected >= ${req.minCount})`);
      }
    }
  }

  console.log(`  Catalog Variety Assertions: ${results.catalogVariety.passed} passed, ${results.catalogVariety.failed} failed.\n`);
  ...
  console.log(`  Food Form Sanity: ${results.foodFormSanity.passed} passed, ${results.foodFormSanity.failed} failed.`);
  console.log(`  Alternatives Richness: ${results.alternativesRichness.passed} passed, ${results.alternativesRichness.failed} failed.\n`);
  ...
  console.log(`  50-Shopping-Lists Audit: ${results.dataset50Lists.passed} / ${results.dataset50Lists.totalLists} lists passed completely.`);
  console.log(`  Evaluated: ${results.dataset50Lists.totalItems} items, ${results.dataset50Lists.totalMatches} store matches, ${results.dataset50Lists.totalAlts} alternatives.\n`);

  // -------------------------------------------------------------------------
  // SUITE 4: Real Browser Playwright End-to-End Visual Proofs & Assertions
  // -------------------------------------------------------------------------
  console.log("▶ SUITE 4: Launching Playwright browser for UI modal assertion and screenshot capture...");
  ...
  try {
    ...
    if (chgBtns.length > 14) {
      ...
      const hasCookingEggs = /6|10|12|15/i.test(modalText) && /free range|medium|large/i.test(modalText);

      if (!hasScotchEgg && hasCookingEggs) {
        results.uiPlaywright.passed++;
        console.log("  ✓ Eggs Swap Modal UI Assertion Passed: 0 Scotch Eggs, rich cooking eggs rendered.");
      } else {
        results.uiPlaywright.failed++;
        console.error("  ✗ Eggs Swap Modal UI Assertion Failed: Scotch eggs found or missing variety.");
      }
    ...
    if (chgBtns2.length > 21) {
      ...
      if (!hasDessertOrDrink && hasGreekYogurtOptions) {
        results.uiPlaywright.passed++;
        console.log("  ✓ Greek Yogurt Swap Modal UI Assertion Passed: 0 dessert drinks, rich Greek yogurt options.");
      } else {
        results.uiPlaywright.failed++;
    ...
    if (chgBtns4.length > 49) {
      ...
      if (!hasCrispsOrChips && hasPotatoOptions) {
        results.uiPlaywright.passed++;
        console.log("  ✓ Potatoes Swap Modal UI Assertion Passed: 0 crisps/chips, rich potato varieties.");
      } else {
        results.uiPlaywright.failed++;
  ...
  console.log("===============================================================================");
  console.log(`1. Catalog Variety Assertions (35 checks across 7 stores): ${results.catalogVariety.passed} PASSED / ${results.catalogVariety.failed} FAILED`);
  console.log(`2. Food Form Sanity (140 store matches evaluated):       ${results.foodFormSanity.passed} PASSED / ${results.foodFormSanity.failed} FAILED`);
  console.log(`3. Alternatives Richness & Sanity:                      ${results.alternativesRichness.passed} PASSED / ${results.alternativesRichness.failed} FAILED`);
  console.log(`4. 50-Shopping-Lists E2E Dataset Audit (2,814 matches):  ${results.dataset50Lists.passed} / 50 PASSED (${results.dataset50Lists.anomalies.length} anomalies)`);
  console.log(`5. Playwright Real UI Modal Assertions:                 ${results.uiPlaywright.passed} PASSED / ${results.uiPlaywright.failed} FAILED`);
  console.log("===============================================================================");
  ...
  if (results.catalogVariety.failed > 0 || results.foodFormSanity.failed > 0 || results.dataset50Lists.failed > 0 || results.uiPlaywright.failed > 0) {
